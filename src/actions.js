...
export const updateProduct = async (product, action) => {
  try {
      let productId = product.map(a => a._id);
      let productQuantity = product.map(a => a.quantity);
      for (let i = 0; i < product.length; i++) {
          const productObj = {productId:productId[i], productQuantity:productQuantity[i]};     
          await Axios.put("/api/products/" + productObj.productId, {
